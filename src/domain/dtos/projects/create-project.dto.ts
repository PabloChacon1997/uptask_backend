...
export class CreateProjectDto {
  ...
  static create(props: {[key: string]: any}): [string, undefined] | [undefined, CreateProjectDto] {
    const { projectName, clientName, description, managerId } = props;
    ...
    if (!managerId || managerId.length === 0) return ['Manager property is required', undefined];
    return [undefined, new CreateProjectDto(projectName, clientName, description, managerId)]
  }
}